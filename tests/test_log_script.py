...
log_mod = load_module_from(LOG_PATH, 'osmanager.system_log.manager_script.log')  # 名称可自定义
...
class TestLogMain(unittest.TestCase):
    ...
    def test_main_success_raw_output(self, mock_run):
        ...
        mock_run.return_value = MagicMock(
            returncode=0,
            stdout="Sep 09 10:23:09 host service[123]: test message\n",
            stderr=""
        )
        ...
        with patch('sys.argv', ['log.py', '--output_format', 'raw']):
            with patch('sys.stdout') as mock_stdout:
                main()
                # mock_stdout.write.assert_called_with("Sep 09 10:23:09 host service[123]: test message\n")
    ...
    def test_argument_parsing(self):
        """测试命令行参数解析"""
        test_cases = [
            (['log.py', '--since', '1h ago', '--priority', 'info'],
             {'since': '1h ago', 'priority': 'info'}),
            (['log.py', '--service', 'nginx', '--limit', '10'],
             {'service': 'nginx', 'limit': 10}),
            (['log.py', '--keyword', 'error', '--identifier', 'sshd'],
             {'keyword': 'error', 'identifier': 'sshd'}),
        ]
                ...
                with patch('subprocess.run') as mock_run:
                    ...
                    with patch('sys.stdout'):
                        ...
                        if 'service' in expected:
                            # self.assertIn('_SYSTEMD_UNIT', call_args)
