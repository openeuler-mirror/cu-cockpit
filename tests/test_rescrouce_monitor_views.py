...
class TestMonitorStatusShModes(TestCase):
    ...
    def test_monitor_all_mode(self, mock_subprocess, mock_isfile):
        ...
        mock_result.stdout = json.dumps({
            "cpu": {"total_utilization_percent": "50%"},
            "disk": {"total_usage_percent": "60%"},
            "memory": {"used_mb": 4096},
            "network": {"interfaces": []}
        })
        mock_result.stderr = ''
    ...
    def test_monitor_cpu_mode(self, mock_subprocess, mock_isfile):
        ...
        mock_result.stdout = json.dumps({
            "cpu": {
                "total_utilization_percent": "45.5%",
                "user_percent": "30%",
                "system_percent": "15%",
                "idle_percent": "55%",
                "load_average": "1分钟: 0.5"
            }
        })
        mock_result.stderr = ''
        mock_subprocess.return_value = mock_result
...
class TestScriptsWithoutMode(TestCase):
    ...
    def test_memory_slot_without_mode(self, mock_subprocess, mock_isfile):
        ...
        if mock_subprocess.called:
            call_args = mock_subprocess.call_args[0][0]
            #alue(len(call_args), 2)  # 只有 ['bash', 'script_path']
...
class TestServiceManagementAPI(TestCase):
    ...
    def test_manage_service_script_not_found(self, mock_isfile):
        ...
        response = self.client.post('/api/rescrouce/service/service_manage.sh',
    ...
    def test_manage_service_success_stop(self, mock_subprocess, mock_isfile):
        ...
        self.assertTrue(response_data['success_flag'])
        self.assertEqual(response_data['service_name'], 'nginx')
        self.assertEqual(response_data['operation'], 'stop')
        self.assertIn('操作成功', response_data['message'])
