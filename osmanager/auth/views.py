...
@swagger_auto_schema(
    ...
    operation_description="使用用户名和密码进行登录验证",
    request_body=openapi.Schema(
        type=openapi.TYPE_OBJECT,
        properties={
            'username': openapi.Schema(type=openapi.TYPE_STRING, description='用户名', example='admin'),
            'password': openapi.Schema(type=openapi.TYPE_STRING, description='密码', example='123456'),
        },
...
def login_view(request):
    ...
    return Response({'code': 400, 'message': '请使用POST请求'},
                  status=status.HTTP_400_BAD_REQUEST)
...
@swagger_auto_schema(
    ...
    operation_description="清除用户会话，退出登录",
    responses={
        200: openapi.Response(
            description="登出成功",
            examples={
...
def logout_view(request):
    try:
        ...
        if 'username' in request.session:
            del request.session['username']  # 删除登录标识
        # 清空整个 session
