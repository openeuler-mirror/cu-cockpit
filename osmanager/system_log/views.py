...
@swagger_auto_schema(
    method='get',
    operation_summary='获取可用的引导偏移列表',
    operation_description='journalctl --list-boots返回可用的引导偏移号数组，如 [0, -1, -2]',
    responses={
        200: openapi.Response(
            ...
            schema=openapi.Schema(
                ...
                properties={
                    'boots': openapi.Schema(
                        type=openapi.TYPE_ARRAY,
                        items=openapi.Schema(type=openapi.TYPE_INTEGER),
                        example=[0, -1, -2, -3]
                    ),
                    'count': openapi.Schema(type=openapi.TYPE_INTEGER, example=4),
                }
            )
        ),
        ...
        500: openapi.Response(
            ...
            schema=openapi.Schema(
                ...
                properties={
                    ...
                    'cmd': openapi.Schema(type=openapi.TYPE_ARRAY, items=openapi.Schema(type=openapi.TYPE_STRING)),
                }
            )
        ),
    },
...
def boots_view(request):
    if not os.path.exists(BOOT_SCRIPT_PATH):
        return JsonResponse({"error": f"boot_offset.py不存在:{BOOT_SCRIPT_PATH}"}, status=404, json_dumps_params={"ensure_ascii": False})

    cmd = [sys.executable or "python3", BOOT_SCRIPT_PATH]
    try:
        proc = subprocess.run(
            cmd,
            capture_output=True,
        ...
        return JsonResponse({"error": "调用boot.py超时(20s)"}, status=504, json_dumps_params={"ensure_ascii": False})
    ...
    try:
        ...
        # 兼容 boot.py打印 Python列表的情况，如 "[0, -1, -2]"
...
@swagger_auto_schema(
    method='get',
    operation_summary='查询系统日志',
    operation_description='根据指定条件查询系统日志，支持按时间、优先级、服务名等条件过滤',
    manual_parameters=[
        openapi.Parameter(
            'output_format',
            openapi.IN_QUERY,
            ...
            enum=['summary', 'all_json'],
            example='summary'
        ),
        openapi.Parameter(
            'cursor',
            openapi.IN_QUERY,
            description='指定cursor获取元数据信息',
            type=openapi.TYPE_STRING,
            example='s=;i=;b=;m=;t=;x='
        ),
        openapi.Parameter(
            'since',
            openapi.IN_QUERY,
            description='开始时间，支持多种格式如 "2025-09-01 10:00:00" 或 "1h ago"',
            type=openapi.TYPE_STRING,
        ...
        openapi.Parameter(
            'until',
            openapi.IN_QUERY,
            description='结束时间，如 "2025-09-01 12:00:00" 或 "now"',
            type=openapi.TYPE_STRING,
            example='2025-08-19 23:59:59'
        ),
        openapi.Parameter(
            'priority',
            openapi.IN_QUERY,
            description='日志优先级，如 "err", "info", "0..7"',
            type=openapi.TYPE_STRING,
            enum=['err', 'info', 'warning', 'debug', 'emerg', 'crit', 'notice', 'alert', '0', '1', '2', '3', '4', '5', '6', '7'],
            example='err'
        ),
        openapi.Parameter(
            'service',
            openapi.IN_QUERY,
            description='服务名/Unit，如 "nginx" 或 "nginx.service"',
            type=openapi.TYPE_STRING,
            example='sshd'
        ),
        openapi.Parameter(
            'identifier',
            openapi.IN_QUERY,
            description='按SYSLOG_IDENTIFIER过滤，如 sshd',
            type=openapi.TYPE_STRING,
            example='sshd'
        ),
        openapi.Parameter(
        ...
        openapi.Parameter(
            'boot',
            openapi.IN_QUERY,
            description='引导选择，"-1" 表示上一引导，"0" 表示当前引导，或指定boot ID',
            type=openapi.TYPE_STRING,
            example='0'
        ),
    ],
    responses={
                ...
                properties={
                    'logs': openapi.Schema(
                        ...
                        items=openapi.Schema(
                            type=openapi.TYPE_OBJECT,
                            properties={
                                'date': openapi.Schema(type=openapi.TYPE_STRING, example='Sep 09'),
                                'time': openapi.Schema(type=openapi.TYPE_STRING, example='10:23:09'),
                                'hostname': openapi.Schema(type=openapi.TYPE_STRING, example='bigdata1'),
                                'service': openapi.Schema(type=openapi.TYPE_STRING, example='PackageKit'),
                                'pid': openapi.Schema(type=openapi.TYPE_INTEGER, example=2177658),
                                'message': openapi.Schema(type=openapi.TYPE_STRING, example='message content'),
                                'raw': openapi.Schema(type=openapi.TYPE_STRING, example='raw log line')
                            }
                        )
                    ),
        ...
        500: openapi.Response(
            ...
            schema=openapi.Schema(
                type=openapi.TYPE_OBJECT,
                properties={
                    'error': openapi.Schema(type=openapi.TYPE_STRING, example='服务内部异常'),
                    'returncode': openapi.Schema(type=openapi.TYPE_INTEGER, example=1),
                    'stderr': openapi.Schema(type=openapi.TYPE_STRING, example='error message'),
                    'cmd': openapi.Schema(type=openapi.TYPE_ARRAY, items=openapi.Schema(type=openapi.TYPE_STRING))
                }
            )
        ),
        504: openapi.Response(
            ...
            schema=openapi.Schema(
                type=openapi.TYPE_OBJECT,
                properties={
                    'error': openapi.Schema(type=openapi.TYPE_STRING, example='调用log.py超时(60s)')
                }
            )
        )
    },
...
def logs_view(request):
    ...
    try:
        proc = subprocess.run(
            ...
            capture_output=True,
            text=True,
            env={**os.environ, "PYTHONIOENCODING": "utf-8", "TZ": "Asia/Shanghai"},
            timeout=60,
            check=False,
        )
    except subprocess.TimeoutExpired:
    ...
    # 合法 JSON：仅接受 list / dict；其他类型视为“非预期 JSON”
